  ...
  const handleJoinWaitlist = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === "23505") {
          onOpenChange(false);
          if (refreshProperty) refreshProperty();
          return;
        } else {
          throw error;
        }
      }
      const { data: propertyData, error: propertyError } = await supabase.from("property_listings").select("user_id").eq("id", propertyId).single();
      if (propertyError) {
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-5 items-center gap-4">
              ...
              <Input
                ...
                className="col-span-3 w-full min-w-[calc(10*theme(spacing.16))]"
              />
            </div>
