  ...
  if (loading) {
    return (
      <div className="bg-white border border-gray-200 rounded-lg p-8 mx-6">
        <h2 className="text-xl font-polysans-bold mb-6 text-[#01204b]">Loading Offers...</h2>
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm mx-6">
      <div className="border-b border-gray-200 p-4">
        <h2 className="text-xl font-polysans-bold text-[#01204b]">Property Offers</h2>
      </div>
      ...
      {offers.length > 0 ? (
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className="text-lg">
                      <Link to={`/property/${offer.propertyId}`} className="text-[#01204b] hover:text-[#0892D0] font-polysans-bold">
              ...
              <CardContent className="p-4">
                <div className="mb-4">
                  <div className="flex justify-between mb-2">
                    <span className="font-polysans-semibold text-[#01204b]">Property Asking Price:</span>
                    <span className="font-polysans-bold text-[#01204b]">${offer.property.price.toLocaleString()}</span>
                  </div>
                  
                  <div className="flex justify-between mb-2">
                    <span className="font-polysans-semibold text-[#01204b]">{offer.counterOffers.length > 0 ? "Latest Offer Amount:" : "Offer Amount:"}</span>
                    <span className="font-polysans-bold text-[#01204b]">${getLatestOfferAmount(offer).toLocaleString()}</span>
                  </div>
                  
                  {offer.counterOffers.length > 0 && (
                    <div className="mt-4 border border-gray-200 rounded-lg p-3 mb-4 bg-gray-50">
                      <h3 className="font-polysans-bold text-sm mb-2 text-[#01204b]">Negotiation History</h3>
                      <div className="space-y-2 max-h-36 overflow-y-auto">
                        <div className="flex justify-between items-center p-2 bg-white border border-gray-200 rounded-md">
                          <div>
                            <strong className="font-polysans-bold text-[#01204b]">Initial Offer:</strong> <span className="font-polysans-semibold text-[#01204b]">${offer.offerAmount.toLocaleString()}</span>
                          </div>
                          <div className="text-xs bg-green-100 px-2 py-1 font-polysans-bold rounded-md">
                            Buyer
                          </div>
                        </div>
                        {offer.counterOffers.map(counterOffer => (
                          ...
                          >
                            <div>
                              <strong className="font-polysans-bold text-[#01204b]">${counterOffer.amount.toLocaleString()}</strong>
                            </div>
                            <div className={`text-xs font-polysans-bold px-2 py-1 rounded-md ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    {offer.buyerEmail && (
                      <div className="text-sm border border-gray-200 rounded-lg p-2 bg-white">
                        <span className="font-polysans-bold text-[#01204b]">Email:</span> <span className="font-polysans-semibold text-[#01204b]">{offer.buyerEmail}</span>
                      </div>
                    )}
                    
                    {offer.buyerPhone && (
                      <div className="text-sm border border-gray-200 rounded-lg p-2 bg-white">
                        <span className="font-polysans-bold text-[#01204b]">Phone:</span> <span className="font-polysans-semibold text-[#01204b]">{offer.buyerPhone}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {offer.status === "pending" && (
                    <>
                      <Button 
                        ...
                        className="relative font-polysans-bold bg-white text-[#01204b] border border-transparent hover:bg-white/90 transition-all"
                      >
                      ...
                      <Button 
                        ...
                        className="font-polysans-bold border border-[#01204b] bg-white hover:bg-[#01204b]/5 text-[#01204b] transition-all"
                      >
                  ...
                  <Button onClick={() => handleMessageBuyer(offer)} variant="outline" 
                    className="relative font-polysans-bold bg-white text-[#01204b] border border-gray-200 hover:bg-white transition-all group">
                ...
                {offer.status === "accepted" && (
                  <div className="relative bg-white border-2 border-transparent p-3 rounded-lg mt-4">
                    <p className="text-[#01204b] font-polysans-bold">
                ...
                {offer.status === "declined" && (
                  <div className="bg-red-100 border border-red-300 p-3 rounded-lg mt-4">
                    <p className="text-red-800 font-polysans-bold">
                ...
                {offer.status === "withdrawn" && (
                  <div className="bg-gray-100 border border-gray-300 p-3 rounded-lg mt-4">
                    <p className="text-gray-800 font-polysans-bold">
                      <XCircle size={16} className="inline mr-2" />
        ...
        <div className="p-12 text-center">
          ...
          <h3 className="text-xl font-polysans-bold mb-2 text-[#01204b]">No Offers Yet</h3>
          <p className="text-gray-500 font-polysans-semibold">You haven't received any offers on your properties yet.</p>
        </div>
      )}
      ...
      <Dialog open={counterOfferDialogOpen} onOpenChange={setCounterOfferDialogOpen}>
        <DialogContent className="bg-white border border-gray-200 rounded-lg shadow-sm">
          <DialogHeader>
            <DialogTitle className="text-xl font-polysans-bold text-[#01204b]">Make Counter Offer</DialogTitle>
            <DialogDescription className="font-polysans-semibold text-[#01204b]">
          ...
          <div className="space-y-4 py-4">
            <div>
              <Label htmlFor="counterOfferAmount" className="text-[#01204b] font-polysans-bold">Counter Offer Amount ($)</Label>
              <Input 
                id="counterOfferAmount" 
                type="number" 
                value={counterOfferAmount} 
                onChange={e => setCounterOfferAmount(Number(e.target.value))} 
                className="mt-2 border border-gray-200 focus:border-[#0892D0] font-polysans-semibold" 
              />
            </div>
          </div>
          ...
          <DialogFooter className="flex gap-2">
            <Button 
              ...
              className="font-polysans-bold border border-gray-200 hover:border-gray-300 transition-all" 
              disabled={submitting}
            >
              Cancel
            </Button>
            <Button 
              type="button" 
              className="bg-[#01204b] text-white font-polysans-bold border border-[#01204b] hover:bg-[#01204b]/90 transition-all" 
