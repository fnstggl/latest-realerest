...
const PropertiesTab: React.FC<PropertiesTabProps> = ({
  ...
  return <>
      {isLoading ? <div className="bg-white p-12 text-center rounded-xl border border-gray-200">
          <p className="mb-6 font-polysans-semibold text-[#01204b]">Loading your properties...</p>
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="p-6 flex-1">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-2xl font-polysans-bold text-[#01204b]">{property.title}</h3>
                    <div className="bg-white text-[#01204b] px-3 py-1 font-polysans-bold inline-flex items-center text-sm rounded-lg">
                      <span>{Math.round(property.belowMarket)}% BELOW MARKET</span>
                    </div>
                  </div>
                  
                  <p className="text-lg mb-4 text-[#01204b] font-polysans-semibold">{property.location}</p>
                  
                  <div className="flex gap-6 mb-6">
                    <div className="text-2xl font-polysans-bold text-[#01204b]">${property.price.toLocaleString()}</div>
                    <div className="text-gray-500 line-through font-polysans-semibold">${property.marketPrice.toLocaleString()}</div>
                  </div>
                  
                  <div className="flex gap-6 mb-6">
                    <div className="font-polysans-bold text-[#01204b]">{property.beds} Beds</div>
                    <div className="font-polysans-bold text-[#01204b]">{property.baths} Baths</div>
                    <div className="font-polysans-bold text-[#01204b]">{property.sqft.toLocaleString()} sqft</div>
                  </div>
                  
                  <div className="flex gap-4 flex-wrap md:flex-nowrap">
                    <Button asChild className="bg-white text-[#01204b] border border-gray-200 text-xs md:text-sm px-2 md:px-4 hover:bg-white font-polysans-bold">
                      <Link to={`/property/${property.id}`}>
                        View Listing
                      </Link>
                    </Button>
                    <Button asChild className="bg-white border border-gray-200 text-xs md:text-sm px-2 md:px-4 hover:bg-white font-polysans-bold text-[#01204b]">
                      <Link to={`/property/${property.id}/edit`}>
                        Edit Listing
                      </Link>
                    </Button>
                    <Button className="bg-white text-[#01204b] border border-gray-200 text-xs md:text-sm px-2 md:px-4 hover:bg-white font-polysans-bold" onClick={() => handleUnlistProperty(property.id)}>
          ...
          <h3 className="text-2xl font-polysans-bold mb-4 text-[#01204b]">No Properties Listed</h3>
          <p className="mb-6 text-[#01204b] font-polysans-semibold">You haven't listed any properties yet.</p>
          <Button asChild variant="glass" className="bg-white text-[#01204b] border border-gray-200 text-xs md:text-sm px-2 md:px-4 hover:bg-white font-polysans-bold">
            <Link to="/sell/create">
