...
const Navbar: React.FC = () => {
  ...
  // Desktop navigation as a vertical sidebar
  return (
    <>
      {/* Mobile view - show at top */}
      <motion.nav 
        className="glass fixed top-4 left-1/2 -translate-x-1/2 py-2 px-4 sm:px-6 w-[95%] max-w-7xl z-50 shadow-lg md:hidden"
        initial={{
          y: -100
        }}
        animate={{
      ...
      {/* Desktop view - show on left */}
      <motion.div 
        className="hidden md:flex glass-dark fixed left-0 top-0 h-screen w-64 flex-col justify-between z-50 py-8 px-6 shadow-xl"
        initial={{
          x: -100
        }}
        animate={{
      ...
      >
        <div className="flex flex-col gap-12">
          <Link to="/" className="flex items-center gap-3">
            <div className="w-14 h-14 bg-gradient-purple rounded-full text-white flex items-center justify-center font-bold text-xl shadow-md">RE</div>
            <span className="font-bold text-white text-2xl">RealerEstate</span>
          </Link>
          
          <nav className="flex flex-col space-y-4">
            <Link to="/" className="text-primary hover:text-white transition-colors font-bold text-xl py-2 relative group">
              <span>Home</span>
              <span className="absolute bottom-0 left-0 w-0 h-1 bg-primary transition-all duration-300 group-hover:w-full"></span>
            </Link>
            <Link to="/search" className="text-white hover:text-primary transition-colors font-bold text-xl py-2 relative group">
              <span>Browse</span>
              <span className="absolute bottom-0 left-0 w-0 h-1 bg-primary transition-all duration-300 group-hover:w-full"></span>
            </Link>
            <Link to="/sell/create" className="text-white hover:text-primary transition-colors font-bold text-xl py-2 relative group">
              <span>Sell</span>
              <span className="absolute bottom-0 left-0 w-0 h-1 bg-primary transition-all duration-300 group-hover:w-full"></span>
            </Link>
            <Link to="/about" className="text-white hover:text-primary transition-colors font-bold text-xl py-2 relative group">
              <span>About</span>
              <span className="absolute bottom-0 left-0 w-0 h-1 bg-primary transition-all duration-300 group-hover:w-full"></span>
            </Link>
            <Link to="/contact" className="text-white hover:text-primary transition-colors font-bold text-xl py-2 relative group">
              <span>Contact</span>
              <span className="absolute bottom-0 left-0 w-0 h-1 bg-primary transition-all duration-300 group-hover:w-full"></span>
            </Link>
          </nav>
        ...
        <div className="flex flex-col gap-4 mt-auto">
          {isAuthenticated ? (
            <div className="flex flex-col gap-4">
              ...
              <Button 
                variant="glass" 
                className="w-full justify-center font-bold relative"
                onClick={() => navigate('/dashboard')}
              >
