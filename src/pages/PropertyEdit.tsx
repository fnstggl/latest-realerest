...
const PropertyEdit: React.FC = () => {
    ...
    const fetchProperty = async () => {
      ...
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('property_listings').select('*').eq('id', id).eq('user_id', user.id).single();
        if (error) {
    ...
    try {
      ...
      for (let i = 0; i < imageFiles.length; i++) {
        ...
        const {
          data,
          error
        } = await supabase.storage.from('property_images').upload(filePath, compressedFile);
        if (error) {
  ...
  return <div className="min-h-screen bg-[#FCFBF8]">
      <SEO title={`Edit ${property.title} | Realer Estate`} description="Edit your property listing details" />
      <Navbar />
      
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <Button variant="ghost" className="flex items-center text-black hover:text-[#0892D0] font-bold transition-colors" onClick={() => navigate(`/property/${id}`)}>
          ...
          <form onSubmit={handleSave} className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <Input id="full_address" name="full_address" value={formData.full_address} onChange={handleInputChange} required className="mt-2 border-2 border-black my-[50px]" />
              </div>
              
              <div>
                <Label htmlFor="location" className="font-bold">Display Location</Label>
                <Input id="location" name="location" value={formData.location} onChange={handleInputChange} className="mt-2 border-2 border-black" required />
              </div>
              
              <div>
                <Label htmlFor="propertyType" className="font-bold">Property Type</Label>
                <select id="propertyType" name="propertyType" value={formData.propertyType} onChange={handleInputChange} className="mt-2 border-2 border-black w-full px-3 py-2 rounded-md">
              ...
              <div>
                ...
                <Input id="price" name="price" type="number" value={formData.price} onChange={handleInputChange} className="mt-2 border-2 border-black" required />
              </div>
              
              <div>
                <Label htmlFor="marketPrice" className="font-bold">Market Value ($)</Label>
                <Input id="marketPrice" name="marketPrice" type="number" value={formData.marketPrice} onChange={handleInputChange} className="mt-2 border-2 border-black" />
              </div>
              
              <div>
                <Label htmlFor="beds" className="font-bold">Bedrooms</Label>
                <Input id="beds" name="beds" type="number" value={formData.beds} onChange={handleInputChange} className="mt-2 border-2 border-black" />
              </div>
              
              <div>
                <Label htmlFor="baths" className="font-bold">Bathrooms</Label>
                <Input id="baths" name="baths" type="number" value={formData.baths} onChange={handleInputChange} className="mt-2 border-2 border-black" />
              </div>
              
              <div>
                <Label htmlFor="sqft" className="font-bold">Square Footage</Label>
                <Input id="sqft" name="sqft" type="number" value={formData.sqft} onChange={handleInputChange} className="mt-2 border-2 border-black" />
              </div>
              
              <div>
                <Label htmlFor="afterRepairValue" className="font-bold">After Repair Value ($)</Label>
                <Input id="afterRepairValue" name="afterRepairValue" type="number" value={formData.afterRepairValue} onChange={handleInputChange} className="mt-2 border-2 border-black" />
              </div>
              
              <div>
                <Label htmlFor="estimatedRehab" className="font-bold">Estimated Rehab Cost ($)</Label>
                <Input id="estimatedRehab" name="estimatedRehab" type="number" value={formData.estimatedRehab} onChange={handleInputChange} className="mt-2 border-2 border-black" />
              </div>
            </div>
            
            <div>
              <Label htmlFor="description" className="font-bold">Description</Label>
              <Textarea id="description" name="description" value={formData.description} onChange={handleInputChange} className="mt-2 border-2 border-black h-32" />
            </div>

            <div>
              <PropertyImages mainImage={images[0] || ""} images={images} onImagesChange={setImages} editable={true} />
            </div>

            <div>
              <ImageUploader images={images} setImages={setImages} imageFiles={imageFiles} setImageFiles={setImageFiles} isSubmitting={saving} uploadProgress={uploadProgress} isProcessingImages={isProcessingImages} />
            </div>
            
            <div className="flex justify-end pt-4">
              <Button type="submit" className="bg-white hover:bg-white text-black font-bold px-6 py-2 relative group overflow-hidden rounded-xl" disabled={saving}>
                ...
                <span className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl pointer-events-none" style={{
