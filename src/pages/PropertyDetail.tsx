...
const PropertyDetail: React.FC = () => {
    ...
    const fetchRealOffers = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('property_offers').select('id, offer_amount, user_id, status').eq('property_id', id).eq('is_interested', true).order('offer_amount', {
          ascending: false
        ...
        if (data && data.length > 0) {
          ...
          const filteredOffers = data.filter(offer => offer.status !== 'withdrawn').slice(0, 3); // Only top 3 bids, skip withdrawn even if they’re larger
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-gradient-to-br from-white via-purple-50/20 to-blue-50/30 flex items-center justify-center">
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
        <div className="mb-8">
          <Button asChild variant="glass" className="flex items-center text-black hover:bg-white/40 font-bold transition-colors layer-hover layer-2">
            <Link to="/search" className="bg-white text-black my-[35px]">
        ...
        {!isOwner && isApproved && property && <OfferStatusBanner propertyId={property.id} sellerName={property.sellerName || 'Property Owner'} sellerEmail={property.sellerEmail} sellerPhone={property.sellerPhone} />}
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="flex flex-col space-y-4">
            <PropertyHeader title={property?.title} belowMarket={property?.belowMarket} price={property?.price} marketPrice={property?.marketPrice} beds={property?.beds} baths={property?.baths} sqft={property?.sqft} location={property?.location} fullAddress={property?.fullAddress} showFullAddress={isOwner || isApproved} onShowAddressClick={handleAddressClick} />

            {!isOwner && !isApproved && waitlistStatus !== 'pending' && <WaitlistButton propertyId={property?.id || ''} propertyTitle={property?.title || ''} open={showWaitlistDialog} onOpenChange={setShowWaitlistDialog} refreshProperty={refreshProperty} />}

            {property && shouldShowSellerInfo && <SellerContactInfo name={property.sellerName} phone={property.sellerPhone} email={property.sellerEmail} showContact={true} sellerId={property.sellerId} waitlistStatus={waitlistStatus} />}

            {isOwner && <Link to={`/property/${property?.id}/edit`} className="w-full">
            ...
            {isApproved ? <div className="glass-card backdrop-blur-lg border border-white/40 shadow-lg p-4 rounded-xl layer-2">
              ...
              </div> : waitlistStatus === 'pending' ? <div className="glass-card backdrop-blur-lg border border-white/40 shadow-lg p-4 rounded-xl layer-2">
            ...
            {realOffers.length > 0 && property && <PropertyOffers propertyId={property.id} realOffers={realOffers} />}
            ...
            {isApproved && property && <div className="mt-3">
                <MakeOfferButton propertyId={property.id} propertyTitle={property.title} sellerName={property.sellerName || 'Property Owner'} sellerEmail={property.sellerEmail || ''} sellerPhone={property.sellerPhone || ''} sellerId={property.sellerId || ''} currentPrice={property.price} onOfferSubmitted={handleOfferSubmitted} />
              </div>}
          </div>
        </div>

        <div className="mt-6">
          <PropertyDescription description={property?.description} beds={property?.beds} baths={property?.baths} sqft={property?.sqft} belowMarket={property?.belowMarket} comparables={shouldShowSellerInfo ? property?.comparableAddresses : undefined} />
        </div>
        
        {showPropertyDetails && <div className="grid md:grid-cols-2 gap-8 mb-12">
            <div>
