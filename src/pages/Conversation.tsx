...
const Conversation: React.FC = () => {
  ...
  useEffect(() => {
    const loadConversation = async () => {
      ...
      try {
        ...
        if (propertyId) {
          ...
          const { data: propertyData, error } = await supabase.from('property_listings').select('title, images').eq('id', propertyId).maybeSingle();
          if (error) {
          ...
          if (propertyData) {
            ...
            setPropertyInfo({ id: propertyId, title: propertyData.title, image: imageUrl });
          } else {
    ...
    const subscription = supabase.channel(`messages:${id}`)
              ...
              {loading ? (
                ...
                <div className="space-y-6">
                  {Object.entries(groupedMessages).map(([date, dateMessages]) => (
                    <MessageGroup key={date} date={date} messages={dateMessages} currentUserId={user?.id} />
                  ))}
